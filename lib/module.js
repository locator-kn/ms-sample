...
routes.push({
    ...
    handler: (request, reply) => {
        ...
        // test if an image format
        if (!/^video\/(?:mp4|mpeg)$/.test(file.hapi.headers['content-type'])) { // TODO: expand regex
...
routes.push({
    method: 'DELETE',
    path: '/stream/video/{videoId}',
    handler: (request, reply) => {

        var id = request.params.videoId;
    ...
    config: {
        ...
        notes: 'Deletes an video with the given ID',
        tags: ['api', 'delete', 'video'],
        validate: {
            params: {
